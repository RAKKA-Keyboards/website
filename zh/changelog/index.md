...
- 添加了一个工具，用于自行调查或向开发者回报断触和误触的问题
- 添加了一个工具，用于通常意义的按键测试
- 修改了一些提示
- 修复查找出的用于切换配置文件的按键有时显示不正确的问题
- 使用的图标字体现在只包含使用到的图标以极大降低下载大小
- 适应最新版本固件的更改
...
- **[60 系列]** 最小分辨 0.01mm，快速触发最低设置为 0.02mm（之前为 0.02mm 0.04mm）
- 修复了上个版本又重新使个别按键出现按一个影响另一个的问题
- 如果快速触发设置得太低而未进行人工底部校准，实际生效的值会被限制在 0.05mm（在配置器上尚未有所提示，限制在多少在未来也可能进行调整）
- 修复当前文件在重新上电后被重置的问题
- 真正完成了 RGB 相关的功能（当然现在的 60 上只有一个灯并不能体现出来）

::: tip 有关性能参数的声明

我们始终保证我们所宣传的参数均是真实有效的，没有虚假宣传或文字游戏。不会出现宣传的精度仅在特定按压距离生效，或使用诸如“8000Hz 回报率就是延迟仅有 0.125ms”之类的误导性表述。

您也可以等待我们在 2024 年末至 2025 年初推出的新产品，它们会有好得多的性能参数。
